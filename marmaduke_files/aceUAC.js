(function() {
 ...
 ACE3NewHost['861839']=100
 ACE3NewHost['847143']=100
 ACE3NewHost['846105']=100
 ACE3NewHost['829817']=100
 ACE3NewHost['861872']=100
 ACE3NewHost['861873']=100
 ACE3NewHost['861864']=100
 ACE3NewHost['847146']=100
 ACE3NewHost['861854']=100
 ACE3NewHost['851828']=100
 ACE3NewHost['836011']=100
 ACE3NewHost['834651']=100
 ACE3NewHost['782102']=100
 ACE3NewHost['790450']=100
 ACE3NewHost['845611']=100
 ACE3NewHost['800700']=100
 ACE3NewHost['825411']=100
 ACE3NewHost['869178']=100
 ...
 var parseSizes = function(sizeStr) {
  var ws = [], hs = [];
  var sz = sizeStr.split(",");
  for (var i = 0; i < sz.length; i++) {
   var str = sz[i].toString();
 ...
 var ACE3_Resize = window["ACE3_Resize"] = function() {
  var adDimensions = window["ACE_SIZES"];
  var iframe = document.getElementById(ACE3_IFRAME_ID);
  var fuzzyMatch = function(w, h) {
   var lowestDistance = 100, matchIdx = 0;
   for (var i = 0; i < adDimensions.widths; i++) {
    var distance = Math.abs(adDimensions.widths[i] - w) + Math.abs(adDimensions.heights[i] - h);
  ...
  var resize = function(w, h, d) {
   var prev = self, cur = parent, currentDepth = 0;
   var reachedTop = false;
   while (!reachedTop && currentDepth < d) {
    reachedTop = cur == top;
    var iframes = cur.document.getElementsByTagName("iframe");
  ...
  var parseSizeComment = function(idx, documentElement) {
   ...
   var re = /(\d+\|\d+)/ig;
   var results = searchString.match(re);
   if (results.length > 0) {
    var lastSize = results[results.length - 1];
    var splitted = lastSize.split("|");
    var width = Number(splitted[0]);
    var height = Number(splitted[1]);
    return {
  ...
  if (iframe) {
   try {
    ...
    if (iframe.width == 0) iframe.width = adDimensions.widths[0];
    if (iframe.height == 0) iframe.height = adDimensions.heights[0];
   } finally {
    iframe.id = "";
   }
  } else {
 ...
 window["ACE3_AdRequest"] = function(obj) {
  ...
  if (old || ct == "IMAGE" || ct == "I") {
   ...
   if (ct == "IFRAME" || ct == "IF") {
    ...
    if (isMultisize) {
     dw += "<SCRIPT>" + "var temp = self.onload;" + "self.onload = function() {" + "if (temp){" + "temp();" + "}" + "ACE3_Resize();" + "}" + "</SCRIPT>";
    }
   }
  }
  if ((pt || pu) && (old || se > -1)) {
